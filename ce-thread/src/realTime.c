/* Reference: https://github.com/nem2564/Real-time-scheduling-algorithms/blob/master/assign2.c */
...
int utilization_test(int total, int set_val)
{
    ...
    for (i = 1; i < set; i++)
    {
        ...
        sum_three = 0;
        sum_one = 0;
        sum_two = 0; // utilizatino for next all elements in tasksets
        for (j = i; j > 0; j--)
        {
        ...
        if (total_sum <= utilization_power)
            printf("Utilization test paseed for subtask %d------\n", i + 1);
        else
        {
            flag = 1;
            failed_task = i;
    ...
    if (!flag)
        printf("ALL TASKS ARE SCHEDULABLE BY UT\n"); // Print result
    return 0;
}
...
int response_time(int total, int set_val)
{
    ...
    for (i = total - set_val + 1; i <= 10; i++)
    {
        ...
        if (responsetime_A[i] == responsetime_A[i - 1])
        { // comdition : if response time for n is == to response time for n-1.
            printf("TASK IS SCHEDULABLE \n");
            //		task_ok++;
            flag = 1;
            break; // break and go out of loop
        }
        else if (responsetime_A[i] > TASK_details[n].deadline)
        { // if deadline over runs then break and declare task to be non schedulable
            flag = 0;
            break;
        }
...
int main()
{
    ...
    while (task_counter != 0)
    {
        ...
        while (task_set_counter != 0)
        {
            ...
            if (TASK_details[line_count].deadline < TASK_details[line_count].period)
                check_for_PD = 1;
